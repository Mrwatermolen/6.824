...
func (ck *Clerk) CreateNullRequest() (id int64, sequenceNum int) {
	ck.sequenceNum++
	return ck.id, ck.sequenceNum
}
...
func (ck Clerk) GetShardData(make_end func(string) *labrpc.ClientEnd, servers []string, configNum int, shard int) GetShardDatareply {
	DPrintf("Clinet %v. GetShardData(). sq: %v. shard: %v.", ck.id, ck.sequenceNum, shard)
	i := 0
	for {
